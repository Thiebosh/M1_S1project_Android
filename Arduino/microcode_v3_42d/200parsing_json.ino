//{b,val,u,mi,M,s} 
double allChannels[8][6] = {
  {0,9.6,0,-2,5,1},       //0
  {1,3.8,1,-6,5,2},       //1
  {0,6.9,0,5,10,1},       //2
  {0,1.4,0,0,5,1},        //3
  {0,6.7,1,2,7,1},        //4
  {1,2.587,0,-0.54,5,0},  //5
  {0,1.02,1,0.5,1.50,2},  //6
  {1,0.25,0,0,1,0}        //7
//{b,val,u,mi,M,s}
}
/*
...
*/
  ...
  const size_t capacityChannel = JSON_OBJECT_SIZE(7);
  DynamicJsonDocument channel(capacity);
  for(int i = 0; i < 8; i++){
    // check if channel i exists or is empty
    channel["id"] = i;
    channel["isActive"] = (allChannels[i][0]==0) ? false : true;  // if (channelActiveStatus & 0x01 << i){true}else{false} || (channelActiveStatus & 0x01 << i) ? true : false;
    channel["currentValue"] = allChannels[i][1];                  // getChannelValue(i);
    channel["unit"] = (allChannels[i][2]==0) ? V : I;             // if(getChannelType(i)==0){channel["unit"] = V; channel["minVoltValue"] = channelMinVoltage[i]; channel["maxVoltValue"] = channelMaxVoltage[i];}
                                                                  // else{channel["unit"] = I; channel["minAmpereValue"] = channelMinCurrent[i]; channel["maxAmpereValue"] = channelMaxCurrent[i];}
    channel["minVoltValue"] = allChannels[i][3];                  // channelMinVoltage[i];
    channel["maxVoltValue"] = allChannels[i][4];                  // channelMaxVoltage[i];
    if(allChannels[i][5]==0){channel["scale"] = _;}
    else if(allChannels[i][5]==1){channel["scale"] = m;}
    else if(allChannels[i][5]==2){channel["scale"] = u;}
    data.add(channel);
  }
